...
<section class="container">
    {#each $Todo_List_CustomStore.filter(todo => todo.status === 'ongoing') as todo (todo.id)}
    <div class="todo-item"  in:scale out:fade={{delay: 250, duration: 300}}>
        ...
        <!-- <input type="checkbox" bind:checked={todo.complete} name="completed" id="completed"> -->
            <Button on:click={()=> Todo_List_CustomStore.completedTask(todo.id)}  type='neutral'>Completed</Button>
